  ...
  return (
    <main className="relative flex min-h-screen flex-col items-center justify-center overflow-hidden bg-white px-6 py-12">
      ...
      <div className="relative z-10 w-full max-w-md space-y-6 text-center">
        ...
        {status === "loading" && <div className="flex justify-center"><div className="h-6 w-6 animate-spin rounded-full border-2 border-neutral-900 border-t-transparent" /></div>}
        {status === "error" && needsLogin && <a href="/auth/login" className="btn btn-primary">Sign in to continue</a>}
        {token && status !== "loading" && (
          <div className="space-y-3">
            <p className="text-sm text-neutral-600">Your CLI should have received this key automatically. If not, copy it and paste it into your terminal:</p>
            <div className="term flex items-center justify-between gap-3 px-4 py-3">
              <code className="t-cyan no-scrollbar overflow-x-auto whitespace-nowrap text-[12px]">{token}</code>
              <button onClick={copyToken} className="btn btn-ghost-dark flex shrink-0 items-center gap-2 px-3 py-1.5 text-xs">
